...
// Copyright (c) 2013 The kfcoin developers
...
uint256 hashGenesisBlock("0x000006e9d56e22a1049cd1299735b8096515ceaad5e2c390fec8386664263ce1");
static const unsigned int timeGenesisBlock = 1392184920;
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // joy joy
...
const string strMessageMagic = "kfcoin Signed Message:\n";
...
static const int64 nTargetTimespan = 60 * 60; // readjust difficulty once per hour
static const int64 nTargetSpacing = 10; // 10 seconds one block
...
{
     /* 8640 blocks/day , 3153600 blocks/year */
    int64 nSubsidy = 1;
	if (nHeight < 100 && nHeight > 0)
    {
        nSubsidy = 0 * COIN;
    }
	else if (nHeight <= 3153600 && nHeight > 100)
    {
        nSubsidy = 1 * COIN;
    }
    else if (nHeight <= 6307200 && nHeight > 3153600)
    {
        nSubsidy = 0.2 * COIN;
    }
	else if (nHeight <= 9460800 && nHeight > 6307200)
    {
        nSubsidy = 0.8 * COIN;
    }
	else if (nHeight <= 12614400 && nHeight > 9460800)
    {
        nSubsidy = 0.2 * COIN;
    }
	else if (nHeight <= 15768000 && nHeight > 12614400)
    {
        nSubsidy = 0.6 * COIN;
    }
	else if (nHeight <= 18921600 && nHeight > 15768000)
    {
        nSubsidy = 0.2 * COIN;
    }
	else if (nHeight <= 22075200 && nHeight > 18921600)
    {
        nSubsidy = 0.4 * COIN;
    }
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000028542fd4edd1fae38410e16afbfe6edbb0155d7d71ed56330926c3933fc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "02 Dec 2013, kfcoin is comming!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x8aac3d42f9dd61281e6098a9b87032151a0de39100749683f9a0e60dbdb98ded"));
...
unsigned char pchMessageStart[4] = { 0xea, 0xe5, 0x3d, 0xde };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("kfcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("kfcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("kfcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kfcoin-miner");
    ...
    try { loop {
        ...
        printf("Running kfcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
